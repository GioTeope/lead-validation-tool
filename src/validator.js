...
function fuzzyMatchesBlocklist(value) {
  ...
  for (const kw of FUZZY_BLOCKLIST) {
    ...
    // Check full value against keyword
    if (levenshtein(s, kw) <= threshold) return kw;
    // Sliding-window check for values longer than the keyword
    // (e.g. "Freelamce Consultant" — check the "freelamce" segment)
    if (s.length > kw.length) {
      for (let i = 0; i <= s.length - kw.length + threshold; i++) {
        const slice = s.slice(i, i + kw.length);
        if (slice.length >= kw.length - threshold && levenshtein(slice, kw) <= threshold) return kw;
      }
    }
  }
  return null;
